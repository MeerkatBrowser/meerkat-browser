...
void Window::triggerAction(int identifier, const QVariantMap &parameters)
{
	...
	switch (identifier)
	{
		...
		case ActionsManager::PrintAction:
			{
				QPrinter printer;
				printer.setCreator(QStringLiteral("Meerkat Browser %1").arg(Application::getFullVersion()));
		...
		case ActionsManager::PrintPreviewAction:
			{
				...
				printPreviewDialog.printer()->setCreator(QStringLiteral("Meerkat Browser %1").arg(Application::getFullVersion()));
